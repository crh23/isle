    ...
    def parent_iterate(
        ...
        if self.operational:
            ...
            for risk in new_nonproportional_risks:
                accept, var_this_risk = self.riskmodel.evaluate(
            ...
            expected_profit, acceptable_by_category, var_per_risk_per_categ = self.riskmodel.evaluate(
            ...
            for categ_id in range(len(acceptable_by_category)):
                ...
                while (
                    acceptable_by_category[categ_id] > 0 and len(categ_risks) > i
                ):  # \
                    ...
                    if (
                        ...
                        if (
                            ...
                            assert (
                                categ_risks[i]["contract"].expiration
                                >= contract.expiration
                            ), "Reinsurancecontract lasts longer than insurancecontract: {0:d}>{1:d} (EXPIRATION2: {2:d} Time: {3:d})".format(
                                contract.expiration,
                                categ_risks[i]["contract"].expiration,
                                categ_risks[i]["expiration"],
                                time,
                            )
                        # else:
                        #    pass
                ...
                not_accepted_risks = [
                    risk for risk in not_accepted_risks if risk.get("contract") is None
                ]
