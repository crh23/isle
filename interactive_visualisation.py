...
def modify_network(doc):
    def make_dataset(time_iter):
        ...
        unweighted_nx_network = nx.from_numpy_array(np.array(unweighted_network_data[time_iter]))
        nx.set_edge_attributes(unweighted_nx_network, network_edge_labels[time_iter], "categ")
        nx.set_node_attributes(unweighted_nx_network, network_node_labels[time_iter], "id")
        ...
        for i in range(number_agent_type[time_iter]['reinsurers']):
            unweighted_nx_network.node[i+number_agent_type[time_iter]['insurers']]['type']='Reinsurer'
        for i in range(number_agent_type[time_iter]['catbonds']):
            unweighted_nx_network.node[i+number_agent_type[time_iter]['insurers']+number_agent_type[time_iter]['reinsurers']]['type']='CatBond'
    ...
    def make_plot(unweighted_nx_network):
        ...
        graph_renderer = from_networkx(unweighted_nx_network, nx.kamada_kawai_layout, scale=1, center=(0,0))
    ...
    def update(attr, old, new):
        ...
        timeselect_slider = Slider(start=0, end=1000, value=new, value_throttled=new, step=1, title="Time", callback_policy='mouseup')
