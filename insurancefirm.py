...
class InsuranceFirm(MetaInsuranceOrg):
    ...
    def ask_reinsurance_non_proportional_by_category(self, time, categ_id, purpose='newrisk'):
    ...
    def issue_cat_bond(self, time, categ_id, per_value_per_period_premium=0):
        ...
        if number_risks > 0:
            ...
            total_premium = sum([per_period_premium * ((1/(1+self.interest_rate))**i) for i in range(risk["runtime"])])
            ...
            contract = ReinsuranceContract(catbond, risk, time, 0, risk["runtime"],
                                           ...
                                           initial_VaR=var_this_risk, insurancetype=risk["insurancetype"])
