...
class MetaInsuranceOrg(GenericAgent):
    ...
    def iterate(self, time):        # TODO: split function so that only the sequence of events remains here and everything else is in separate methods
        ...
        if self.operational:
            ...
            new_nonproportional_risks, new_risks = self.get_newrisks_by_type(contracts_dissolved)
    ...
    def obtain_yield(self, time):
        """Method to calculate interest generated by agents total cash
            Accepts:
                time: Type integer
            No return value"""
    ...
    def get_newrisks_by_type(self, contracts_dissolved):
        """Method for soliciting new risks from insurance simulation then organising them based if non-proportional
            or not.
           Accepts:
                contracts_dissolved: list of contracts dissolved, needed for verbose condition.
            Returns:
