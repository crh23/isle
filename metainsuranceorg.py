...
class MetaInsuranceOrg(GenericAgent):
    def __init__(self, simulation_parameters: dict, agent_parameters: AgentProperties):
        ...
        self.reinsurance_history = [[] for _ in range(12)]
    ...
    def market_permanency(self, time: int):
        ...
        if not self.simulation_parameters["market_permanency_off"]:
            ...
            if self.cash < self.simulation_parameters["cash_permanency_limit"]:
            ...
            if self.is_insurer:
                for contract in maturing_next:
                    ...
                    if (
                        ...
                        self.simulation.return_risks([contract.risk])
                    else:
    ...
    def consider_buyout(self, firm_type="insurer"):
        ...
        for firm, time, reason in firms_to_consider:
            ...
            firm_price = (
                total_premium
                + ddm_stock
                + (np.mean(firm.cash_last_periods[0:12]) ** 2) / all_firms_cash
            )  # Price based on ddm stock value, cash flow from premiums, and market capitalization
            if self.excess_capital > firm.var_sum + all_obligations - total_premium:
                firm_likelihood = (
                    0.25
                    + (
                        1.5 * firm.cash
                        + np.mean(firm.cash_last_periods[0:11])
                        + self.cash
                    )
                    / all_firms_cash
                )  # Likelihood depends on size of other firm and itself
    ...
    def submit_regulator_report(self, time):
        ...
        condition = self.simulation.bank.regulate(
            ...
            self.age,
            self.riskmodel.margin_of_safety,
        )
        if condition == "Good":
            self.warning = False
